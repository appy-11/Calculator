...
function num(val){
   val = val.toString(); // so that it read 2+2 = 4 not 22
  
   if (cal_done){clr();}//clear if calcualtion is done\
   if(!operPressed){
       //first value i.e no operator is pressed
       if(!val_1) val_1=0;
   ...
   if(operPressed){
       //second value i.e  operator is pressed
       if(!val_2) val_2=0;
...
function calc(val){
   ...
   if( !val_1 || !val_2){
       result.innerHTML = val; //shows the operator
...
//function to fix length of the result
